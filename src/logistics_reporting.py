...
DAILY_REPORT_HEADERS = [
    "Report Date", "Agent", "Assigned", "Handled Orders", "Activities",
    "Calls Logged", "Answered Calls", "WhatsApp Actions", "Follow-ups Scheduled",
    "Rescheduled", "Cancelled Responses", "Closed", "Recovered", "Recovery Rate",
    "Generated At",
]
...
def resolve_date_window(view: str, custom_start: date, custom_end: date) -> tuple[date | None, date | None]:
    ...
    if view == "Yesterday":
        ...
        return day, day
    if view in windows:
        return today - timedelta(days=windows[view]), today
    if view == "Custom Range":
        return min(custom_start, custom_end), max(custom_start, custom_end)
    return None, None


def filter_cases_by_date(cases: pd.DataFrame, basis: str, start_date: date | None, end_date: date | None) -> pd.DataFrame:
...
def daily_agent_report(cases: pd.DataFrame, activity: pd.DataFrame, report_date: date) -> pd.DataFrame:
    ...
    for agent in AGENTS:
        ...
        rows.append({
            "Report Date": report_date.isoformat(), "Agent": agent, "Assigned": assigned,
            "Handled Orders": handled, "Activities": activities, "Calls Logged": calls,
            "Answered Calls": answered, "WhatsApp Actions": whatsapp,
            "Follow-ups Scheduled": followups, "Rescheduled": rescheduled,
            "Cancelled Responses": cancelled, "Closed": closed, "Recovered": recovered,
            "Recovery Rate": recovered / handled if handled else 0.0,
        })
    return pd.DataFrame(rows)


def daily_case_details(cases: pd.DataFrame, activity: pd.DataFrame, report_date: date, agent: str | None = None) -> pd.DataFrame:
    activity_work = activity.copy()
    activity_work["_action_date"] = _dates(activity_work.get("Action At", pd.Series(dtype=str)))
    day_activity = activity_work[activity_work["_action_date"].eq(report_date)].copy()
    if agent:
        day_activity = day_activity[day_activity["Logistics Agent"].astype(str).str.upper().eq(agent.upper())]
    ...
    for case_id, group in day_activity.groupby("Case ID", dropna=False):
        ...
        last = group.iloc[-1]
        summaries.append({
            "Case ID": _text(case_id), "Agent": _text(last.get("Logistics Agent")),
            "Activities": len(group), "Calls": int(group["Action Type"].astype(str).str.upper().eq("CALL").sum()),
            "Answered": int(group["Call Result"].astype(str).str.casefold().eq("answered").sum()),
            "Latest Action At": _text(last.get("Action At")), "Latest Action": _text(last.get("Action Type")),
            "Last Call Result": _text(last.get("Call Result")),
            "Last Customer Response": _text(last.get("Customer Response")),
            "Latest Remark": _text(last.get("Remark")), "Next Follow-up": _text(last.get("Next Follow-up")),
        })
    detail = pd.DataFrame(summaries)
    case_columns = [
        "Case ID", "Portal", "AWB", "Customer Name", "Mobile", "Latest Courier Status",
        "Tawseel Status Updated At", "Logistics Work Status", "Logistics Final Outcome",
        "Delivered After Coordination",
    ]
    available = [column for column in case_columns if column in cases.columns]
    return detail.merge(cases[available].drop_duplicates("Case ID", keep="last"), on="Case ID", how="left")
...
def _daily_report_sheet() -> gspread.Worksheet:
    ...
    try:
        ...
        worksheet = book.add_worksheet(title="LOGISTICS_DAILY_REPORT", rows=5000, cols=len(DAILY_REPORT_HEADERS))
